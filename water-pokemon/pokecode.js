...
/** function loadPage() {
    getAPIData(`https://pokeapi.co/api/v2/type`).then
    (async (data) => {
        for (const type of data.results) {
            await getAPIData(type.url).then((pokeData) => {
                populatePokeCard(pokeData)
            })
        }
    })
...
loadButton.addEventListener('click', () => {
    ...
    (async (data) => {
        ...
        waterImage.src = `../pokemon/images/${getImageFileName(pokemon)}.png`
        pokemonGrid.appendChild(waterImage)
    })
})


//end of water button code
